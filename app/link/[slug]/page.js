  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ background: '#2E8FF0', padding: '20px' }}>
        ...
        {unit && sessionActive && (
          <div style={{ marginTop: '8px', display: 'inline-block', background: 'rgba(255,255,255,0.2)', borderRadius: '20px', padding: '4px 12px' }}>
            <span style={{ color: '#fff', fontSize: '12px', fontWeight: '600' }}>Delivering to Unit {unit}</span>
          </div>
        )}
      </div>
      {building.notes && (
        <div style={{ background: '#fffbeb', borderBottom: '1px solid #fde68a', padding: '12px 20px' }}>
          <p style={{ fontSize: '14px', color: '#92400e' }}>📝 {building.notes}</p>
        </div>
      )}
      {showCodes && (building.gate_code || building.lobby_code) && (
        <div style={{ background: '#EBF4FE', borderBottom: '1px solid #bfdbfe', padding: '16px 20px' }}>
          ...
          <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap' }}>
            {building.gate_code && (
              <div>
                <p style={{ fontSize: '12px', color: '#666', marginBottom: '2px' }}>Gate code</p>
                <p style={{ fontFamily: 'monospace', fontSize: '24px', fontWeight: '700', color: '#1A5FA8', letterSpacing: '3px' }}>{building.gate_code}</p>
              </div>
            )}
            {building.lobby_code && (
              <div>
                <p style={{ fontSize: '12px', color: '#666', marginBottom: '2px' }}>Lobby code</p>
                <p style={{ fontFamily: 'monospace', fontSize: '24px', fontWeight: '700', color: '#1A5FA8', letterSpacing: '3px' }}>{building.lobby_code}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ maxWidth: '500px', margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #eee', textAlign: 'center' }}>
          <p style={{ fontWeight: '800', color: '#666' }}>
            Find<span style={{ color: '#2E8FF0' }}>Found</span>Fast
          </p>
        </div>
      </div>
    </div>
